...
const useDropdown = (
  ...
  const Dropdown: FC<DropdownProps> = ({ children, className, parentNode }) => {
    return isOpen
      ? createPortal(
          <div
            style={menuStyle}
            ref={menuRef}
            className={
              "menu dropdown-content rounded-box bg-base-100 absolute z-[1] max-h-[50vh] flex-col flex-nowrap overflow-y-scroll p-2 shadow-xl" +
              (className ? ` ${className}` : "")
            }
