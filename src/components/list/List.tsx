...
import { removeWindow } from "../data/actions"
...
const List: React.FC<ListProps> = ({
	...
	const { copy, paste } = useOperationsContextSelector((v) => ({
		copy: v.copy,
		paste: v.paste,
	}))
	// const { settings } = useSettingsContextSelector()
			...
			>
				...
				{type === "collection" && (
					<>
						...
						<div
							className='tooltip'
							data-tip='paste to window'
						>
							<button
								className='btn btn-xs m-1'
								onClick={() => paste(window)}
							>
								<Icon
									Svg={PasteIcon}
									width={4}
