...
import { useOperationsContextSelector } from "@/components/operations/operationsContext"
...
const CollectionOperations = ({
	...
	const {
		...
		goToWindow: v.goToWindow,
		deleteWindow: v.deleteWindow,
		openCollection: v.openCollection,
		saveCurrentToCollection: v.saveCurrentToCollection,
		deleteCollection: v.deleteCollection,
	}))
...
const SelectedOperations = ({ current, type, collections, allTabsNumber }) => {
	const {
		...
		setSelectedList: v.setSelectedList,
		openSelected: v.openSelected,
		deleteSelected: v.deleteSelected,
		addSelectedToCollection: v.addSelectedToCollection,
		copy: v.copy,
	}))
...
const ContentLayout = memo(({ current, type, children }) => {
	...
	const { setSelectedList } = useOperationsContextSelector(
		(v) => v.setSelectedList
	)
	const { setCollectionTitle } = useOperationsContextSelector(
		(v) => v.setCollectionTitle
	)
...
const Content = () => {
	...
	const { tabsByWindowMap, onSelect, setTabsByWindow } =
		useOperationsContextSelector((v) => ({
			tabsByWindowMap: v.tabsByWindowMap,
			onSelect: v.onSelect,
			setTabsByWindow: v.setTabsByWindow,
		}))

	// 使用 useMemo 优化 windowList 计算，只在 current 变化时重新计算
	// BUG: add usememo will cause dnd updated display failed
	const windowList =
		// useMemo(() => {
		// return
		current?.windows ?? []
